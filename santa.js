...
var nPresent = 0; // 전체 선물 수
var nClickPres = 1; // 선물상자를 클릭했을 때 생산되는 선물 수
var aItem = []; // 아이템
var nAddiIncome = 1; // 추가 해고수당

var oBgm = new Audio("sound/exploited_rudolph.wav");
...
function openPopupWindow(sUrl) {
    var nWidth = 600;
    var nHeight = 500;
    var nPosX = (screen.availWidth - nWidth)/2;
    var nPosY = (screen.availHeight - nHeight)/2;
    var sOption = "width=" + nWidth + ", height=" + nHeight + ", top=" + nPosY + ", left=" + nPosX + "resizable=no, scrollbars=no, status=no;";

    window.open("popup/" + sUrl, "", sOption);
}
...
function updatePolicy(sPolicy) {
    if(sPolicy == "practice") { // 리본묶기
        ...
        htItem[0]["prod"] += 1;
        $("#dolf .pr").text(htItem[0]["addi"] + " ~ " + htItem[0]["prod"] + " мобил/сек");
        renderPolicyList(3); // 열정페이 오픈
        renderPolicyList(4); // 멀티태스킹 오픈
    } else if(sPolicy == "pashion") { // 열정페이
        htItem[1]["prod"] += 1;
        $("#elf .pr").text(htItem[1]["addi"] + " ~ " + htItem[1]["prod"] + " мобил/сек");
        renderPolicyList(5);
        ...
        htItem[2]["prod"] += 1;
        $("#elf2 .pr").text(htItem[2]["addi"] + " ~ " + htItem[2]["prod"] + " мобил/сек");
        ...
        for(var i = 0; i < aItem.length; i++) {
            var nUnion = Math.floor(Math.random() * 2);
            aItem[i].prod += 1;
            if(nUnion == 0) {
                aItem[i].union = 1;
                aItem[i].prod = nUnion;
...
function renderItemList(n) {
    ...
    $(ITEM_LIST).append("<li id='" + htItem[n]["name"] + "'>" +
        ...
        "<p>" + htItem[n]["k_name"] + "<img class='item-present-img' src='img/present.png'>" +
            "<span class='t'>" + htItem[n]["cost"] + "</span>" +
            "<br/><span class='pr'>" + htItem[n]["addi"] + " ~ " + htItem[n]["prod"] + " мобил/сек</span>" +
        "</p>" +
    "</li>");
    ...
    $(ITEM_LIST).append("<li class='preview'>" +
        ...
        "<p>???<img class='item-present-img' src='img/present.png'>" +
            "<span class='t'>" + htItem[n+1]["cost"] + "</span>" +
            "<br/><span class='pr'>??? ~ ??? мобил/сек</span>" +
        "</p>" +
    "</li>");
}
...
function renderPolicyList(n) {
    $("#policy-list").append("<li>" +
        "<p>" + htPolicy[n]["k_name"] + "<img class='item-present-img' src='img/present.png'>" +
    ...
    for(var i = 0; i < aItem.length; i++) {
        if(aItem[i].union == 0) {
            $(MANAGE_LIST).append("<li id='" + aItem[i].name + "'>" +
                ...
                "<p>" + aItem[i].k_name + "<br/>В сек: <img class='item-present-img' src='img/present.png'>" +
            ...
            $(MANAGE_LIST).append("<li id='" + aItem[i].name + "'>" +
                "<img class='item-img' src='" + aItem[i].img + "'/>" +
                "<p>" + aItem[i].k_name + "<span style='color:orangered;font-size:10pt;'> 노조상태!</span><br/>В сек: <img class='item-present-img' src='img/present.png'>" +
                "<span class='t'>" + aItem[i].prod + "</span>" +
                "</p>" +
                "</li>");
        }
...
function updateGame() {
    ...
    $("#present-info").text("Скорость: " + nAllProd + " мобил в секунду!");
    updatePresent(nAllProd);
}

var nX = [300, 300, 300, 300, 300, 300, 300, 300]; // 출력 x좌표
var nAddiX = [30, 75, 75, 75, 75, 75, 75, 75]; // 추가할 x좌표 값
var nDefaultY = window.innerHeight / 7;

// 캔버스에 아이템을 출력한다.
function renderItem(sImage, nIndex) {
    var nY = nDefaultY * nIndex + 30;
...
$("#manage-list").delegate("li", "click", function() {
    var nIndex = $(this).index();
    var nIncome = aItem[nIndex].prod * 10 * nAddiIncome;

    if(aItem[nIndex].union == 0) {
        if(confirm(aItem[nIndex].k_name + aItem[nIndex].dstory + "\nТочно продаёшь?\nТебе вернут " + nIncome + " Телефонов.")) {
